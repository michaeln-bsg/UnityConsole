...
namespace Rubycone.UConsole {
	...
	public class UConsoleInputField
		...
		public event Action OnTabEntered;
		...
		public string userText {
			get {
				return text.Substring(fieldHeader.Length);
			}
		}
		...
		protected void ClampPos(ref int pos) {
			if(pos < fieldHeader.Length) pos = fieldHeader.Length;
		...
		public void MoveTextStart(bool shift) {
			int position = fieldHeader.Length;
		...
		public virtual void OnUpdateSelected(BaseEventData eventData) {
			...
			while(Event.PopEvent(m_ProcessingEvent)) {
				if(m_ProcessingEvent.rawType == EventType.KeyDown) {
					...
					else if(shouldContinue == EditState.Submit)
						break;
				}
			}
		...
		private void Backspace() {
			if(hasSelection) {
				...
				if(caretPosition > fieldHeader.Length) {
